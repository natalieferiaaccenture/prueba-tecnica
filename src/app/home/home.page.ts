...
export class HomePage implements OnInit, OnDestroy {
  ...
  public newTask: Task = { id: 0, name: '', category: '', date: new Date(), completed: false };
  ...
  constructor() {
    this.filteredTasks$ = combineLatest([this.tasks$, this.categories$, this.selectedCategory$])
      .pipe(
  ...
  private loadTasksFromLocalStorage(): void {
    ...
    if (storedTasks) {
      this.tasksSubject.next(JSON.parse(storedTasks).map((task: any) => ({
        ...task,
  ...
  private saveCategoriesToLocalStorage(): void {
    localStorage.setItem(this.categoriesLocalStorageKey, JSON.stringify(this.categoriesSubject.value));
  }

  private loadCategoriesFromLocalStorage(): void {
    const storedCategories = localStorage.getItem(this.categoriesLocalStorageKey);
    if (storedCategories) {
      this.categoriesSubject.next(JSON.parse(storedCategories));
    } else {
      this.categoriesSubject.next(['Personal', 'Trabajo', 'Salud', 'Estudios', 'Hogar', 'Finanzas', 'Ocio']);
  ...
  deleteCompletedTasks(): void {
    const incompleteTasks = this.tasksSubject.value.filter(task => !task.completed);
    this.tasksSubject.next(incompleteTasks);
    this.saveTasksToLocalStorage();
  }

  openAddTaskModal(): void {
    this.newTask = { id: 0, name: '', category: '', date: new Date(), completed: false };
  ...
  addCategory(): void {
    if (this.newCategoryName && !this.categoriesSubject.value.includes(this.newCategoryName)) {
      this.categoriesSubject.next([...this.categoriesSubject.value, this.newCategoryName]);
      this.saveCategoriesToLocalStorage();
      this.newCategoryName = '';
  ...
  deleteCategory(categoryToDelete: string): void {
    if (confirm(`¿Seguro que quieres eliminar la categoría "${categoryToDelete}"? Las tareas asociadas a esta categoría se mantendrán sin categoría.`)) {
      const updatedCategories = this.categoriesSubject.value.filter(cat => cat !== categoryToDelete);
      this.categoriesSubject.next(updatedCategories);
      this.saveCategoriesToLocalStorage();
      // Opcional: Puedes actualizar las tareas que tenían esta categoría a una categoría por defecto o vacía.
      const updatedTasks = this.tasksSubject.value.map(task =>
  ...
  getIndicatorColor(category: string): string {
    switch (category) {
      case 'Personal': return '#f44336';
      case 'Trabajo': return '#ff9800';
      case 'Salud': return '#4caf50';
      case 'Estudios': return '#9c27b0';
      case 'Hogar': return '#03a9f4';
      case 'Finanzas': return '#fdd835';
      case 'Ocio': return '#e040fb';
